...
# This Streamlit page provides a combined view over Downstream Tasks and Datasets.
# Users can apply filters to both tasks and datasets and then correlate them through
# their "enabled_by" relationships. The underlying database contains an ``Edges``
# collection where entries with ``relation_type`` set to ``enabled_by`` link a task
# (via the ``from`` field) to a dataset (via the ``to`` field).
...
if query:
    ...
    if joined_results:
        ...
        st.info(f"Datasets: {', '.join(sorted(dataset_names))}")
    else:
        ...
        st.info("Try adjusting your filters or selecting different downstream tasks. "
               "Available relationships exist for: Text Summarization and Code Generation.")
