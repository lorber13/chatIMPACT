...
# with less than 8B parameters trained on Italian medical fine-tuning datasets.
# The query is automatically executed when the page loads.

PAGE = "Query 1 - Medicine"
MODELS = "Models"
DATASETS = "Datasets"
DB_NAME = "ChatIMPACT"
...
if f"{PAGE}.initialized" not in st.session_state:
    ...
    st.session_state[f"{PAGE}.fine_tuning"] = "True"
    st.session_state[f"{PAGE}.domain"] = ["Medical"]
    st.session_state[f"{PAGE}.lan_ds"] = ["Italian"]
...
with col_12:
    ...
    st.multiselect(
        ...
        default=st.session_state.get(f"{PAGE}.domain", ["Medical"]),  # Pre-select "Medical"
        key=f"{PAGE}.domain"
    )
...
with col_14:
    ...
    st.multiselect(
        ...
        default=st.session_state.get(f"{PAGE}.lan_ds", ["Italian"]),  # Pre-select "Italian"
        key=f"{PAGE}.lan_ds"
    )
    st.multiselect(
        "**LicenseToUse**",
...
# Set up filters for LLMs
st.session_state[f"{PAGE}.filters_llm"] = {}

# Add openSource filter
...
# State flags for additional manual filtering
...
if query:
    ...
    if st.session_state.get(f"{PAGE}.rank_by", "No ranking") == "Model Parameters" and "numberOfParameters" not in llm_project_fields:
        llm_project_fields.append("numberOfParameters")
    elif st.session_state.get(f"{PAGE}.rank_by", "No ranking") == "Model Carbon Emissions" and "carbon_emissions_tco2e" not in llm_project_fields:
        llm_project_fields.append("carbon_emissions_tco2e")
    ...
    # -------------------------------------------------------------------------
    # Combine models and datasets according to the edges
    joined_results = []
    ...
    if joined_results:
        ...
        if st.session_state.get(f"{PAGE}.rank_by", "No ranking") != "No ranking":
            ...
            def has_valid_ranking_value(item):
                ...
                if st.session_state.get(f"{PAGE}.rank_by", "No ranking") == "Model Parameters":
                    if "Models" in item and "numberOfParameters" in item["Models"]:
                        param_value = item["Models"]["numberOfParameters"]
                        return convert_params_to_numeric(param_value) is not None
                    return False
                elif st.session_state.get(f"{PAGE}.rank_by", "No ranking") == "Model Carbon Emissions":
                    if "Models" in item and "carbon_emissions_tco2e" in item["Models"]:
                        try:
                            ...
                            return False
                    return False
                elif st.session_state.get(f"{PAGE}.rank_by", "No ranking") == "Dataset Size":
                    if "Datasets" in item and "size" in item["Datasets"]:
                        size_value = item["Datasets"]["size"]
        ...
        st.success(f"Found {len(joined_results)} model-dataset relationships matching the criteria.")
        ...
        st.warning("No training relationships found matching the specified criteria.")
        st.info("This means there are no open-source models with less than 8B parameters that have been trained on Italian medical fine-tuning datasets in the current database.")
