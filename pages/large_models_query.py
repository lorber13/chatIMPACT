...
if f"{PAGE}.initialized" not in st.session_state:
    ...
    st.session_state[f"{PAGE}.min_num_param"] = 70.0
    ...
    st.session_state[f"{PAGE}.domain"] = []
    st.session_state[f"{PAGE}.lan_ds"] = ["English"]
    st.session_state[f"{PAGE}.lic"] = []
    
    # Display options
    st.session_state[f"{PAGE}.project_llm_multiselect"] = ["name", "version", "numberOfParameters", "carbon_emissions_tco2e"]
    st.session_state[f"{PAGE}.project_ds_multiselect"] = ["name", "uri", "languages"]
    
    # Ranking options - pre-configured for carbon emissions ascending
    st.session_state[f"{PAGE}.rank_by"] = "Model Carbon Emissions"
    st.session_state[f"{PAGE}.sort_order"] = "Ascending (Low to High)"
...
st.image("static/5.png", use_column_width=True)
...
with col_2:
    ...
    if st.session_state[f"{PAGE}.num_param_filter"] == "Number of Parameters (B)":
        st.number_input(
            "**Minimum number of parameters (B)**", min_value=0.0, value=70.0, step=0.1,
...
if st.session_state[f"{PAGE}.num_param_filter"] == "Number of Parameters (B)":
    # When using numeric filters on a string field (e.g. "70B"), we can't rely on
...
with col_12:
    ...
    st.multiselect(
        ...
        key=f"{PAGE}.domain",
        default=st.session_state.get(f"{PAGE}.domain", []),
    )
...
with col_14:
    # language and license filters for datasets - pre-configured for English
    st.multiselect(
        "**Language**",
        dao.get_all("Datasets", "languages"),
        key=f"{PAGE}.lan_ds",
        default=st.session_state.get(f"{PAGE}.lan_ds", ["English"]),
    )
    st.multiselect(
        "**LicenseToUse**",
...
st.multiselect(
    ...
    st.session_state.get(f"{PAGE}.project_llm_multiselect", ["name", "version", "numberOfParameters", "carbon_emissions_tco2e"]),
...
st.multiselect(
    ...
    st.session_state.get(f"{PAGE}.project_ds_multiselect", ["name", "uri", "languages"]),
    key=f"{PAGE}.project_ds_multiselect",
)

with st.expander("**📊 Ranking Options**", expanded=True):  # Expanded by default since ranking is key for this query
    st.radio(
        "**Rank by**",
        ["No ranking", "Model Parameters", "Model Carbon Emissions", "Dataset Size"],
        index=2,  # Pre-select "Model Carbon Emissions"
        key=f"{PAGE}.rank_by"
    )
    if st.session_state[f"{PAGE}.rank_by"] != "No ranking":
        st.radio(
            ...
            index=0,  # Pre-select "Ascending" for lowest carbon emissions first
...
if query:
    ...
    # -------------------------------------------------------------------------
    # Query for the edges that link models to datasets via training relationships
    ...
    # -------------------------------------------------------------------------
    # Combine models and datasets according to the edges. We strip the 
    ...
    for edge in edges_result:
        ...
        # if both sides match, add to results
    ...
    if joined_results:
        ...
        st.info("This means there are no models with more than 70B parameters trained on English datasets in the current database.")
